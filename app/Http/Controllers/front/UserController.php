...
//            Session::flash('success',   "اطلاعات کاربری با موفقیت به روز رسانی شد");
    ...
    {
        $orders = Order::whereIn('address_id', Auth::user()->addresses()->pluck('id'))
            ->get();

        return view('front.User.userOrders', compact('orders'));
    }

    public function order_details($id)
    {
        $order = Order::whereIn('address_id', Auth::user()->addresses()->pluck('id'))
            ->whereId($id)
            ->firstOrFail();

        return view('front.User.userOrderDetails', compact('order'));
    }
